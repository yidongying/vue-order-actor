<template>
   <div class="box">
       <div class="banner-list">
           <!-- 上传图片文件 -->
          <!-- <uploader :max="varmax" :images="ktvImages"  :show-header="false"  :upload-url="uploadUrl"  size="small"  
             @preview="previewMethod"  @add-image="addImageMethod" @remove-image="removeImageMethod" ></uploader> -->
            <swiper loop auto :list="ktvImages" ></swiper>
       </div>
    <div class="splite-list">
        <div class="all-list" >
            <span v-if="!cityChose"  class="all-title"  @click="citySelect()">全部 <i></i></span> 
            <span v-else class="all-title" @click="citySelect()">{{currentCity}} <i></i></span>
            <!-- 通过v-show="showChose"来判断是否显示地区选择 -->
            <div v-show="this.showChose" class="citySelect">
                <section class="showChose" >
                    <section class="address">
                        <section class="title headertitle">
                        <h3 @click="provinceSelected()">全部</h3>
                        <span @click="closeAdd()">×</span>
                        </section>
                        <section class="title">
                        <div class="area" @click="provinceSelected()">{{Province?Province:info[province-1].name}}</div>
                        <div class="area" @click="citySelected()" :class="City?'':'active'">{{City?City:'城市'}}</div>
                        <div class="area" @click="districtSelected()" :class="District?'':'active'" v-show="City">{{District?District:'地区'}}</div>
                        </section>
                        <ul>
        ...
        <div class="icon-search" @click="searchServer"></div>
    </div>
    <list :ktv-list="ktvList" :page-config="pageConfig" :city="currentCity" />
    <div v-show="this.ktvList.length<=0" class="noneKtv">暂无数据</div>
</div>
</template>
...
<script>
    ...
    const ktvList=[
         {
            img:"../../../../static/images/home-list/list1.png",
            title:"尚辰量贩KTV",
            addr:"萝岗区科丰路89号萝岗万达广场",
            discounts:"168元购晚场欢唱套餐",
            number:"0201",
            id:1
        },
         {
    ...
    export default{
        ...
        data(){
           return {
                ...
                ktvList,
                // ktvList:[],
                // isDectails:false,  //判断是否点击了ktv详情页，通过点击ktv标题，ktv图片等触发
                currentCity:'广州',
                pageConfig:{
                    size:3,
                    page:1,
                ...
                cityChose:false,  //判断是否选择了城市
                showChose:false,  //判断是否需要显示城市列表，通过点击“全部”触发
                showProvince: true,//判断是否需要显示省份，默认显示省份
                showCity: false,  //判断是否显示该省份下的城市
                showDistrict: false,
                showCityList: false,
        ...
        created(){
            //模拟用户登录 (get)   首次加载页面时获取token值
                this.$ajax({
                    url:this.$store.state.url+'/api/manager/test',
                    method:'get',
                }).then(res=>{
                    var  userToken=null;
                    if(res.status==200){
                        // console.log(res.data.parms.token);
                        userToken=res.data.parms.token;
                    }
                    localStorage.setItem("userToken",userToken);  //将后台获取的token保存在本地;
                });
            //模拟艺人登录（get）
                this.$ajax({
                    url:this.$store.state.url+'/api/manager/test0',
                    method:'get',
                }).then(res=>{
                    var  actorToken=null;
                    if(res.status==200){
                        console.log(res.data.parms.token);
                        actorToken=res.data.parms.token;
                    }
                    localStorage.setItem("actorToken",actorToken);  //将后台获取的token保存在本地;
                });
        },
        mounted(){
              //    this.getData();
        ...
        methods:{
            getData(){
                //查询当前城市下的KTV
                this.$ajax({
                    url:this.$store.state.url+"/api/ktv/findc/"+this.currentCity+'?page='+this.pageConfig.page+'&size='+this.pageConfig.size,
                    method:'post',
                    data:{}
                }).then((res)=>{
                    // console.log(res.data.parms.ktvs);
                    if(res.status==200){
                        this.ktvList.title=res.data.parms.ktvs.name;  //ktv名称
                        this.ktvList.addr=res.data.parms.ktvs.addr;  //地址
                        this.ktvList.discounts=res.data.parms.ktvs.discounts;  //折扣
                        this.ktvList.remark=res.data.parms.ktvs.remark;   //公告
                        this.ktvList.id=res.data.parms.ktvs.id;   //KTVId
                        this.pageConfig.maxsize=res.data.parms.ktvs.maxSize   //最大页数
                        // this.ktvList.number=res.data.parms.ktvs.  ?
                    }
                });
            },
            citySelect(city){
  ...
  <style lang="scss"> 
        .box{margin:0 auto;}
      .splite-list{height:1.5rem;display:flex;align-items:center;text-indent: 0.2rem;border-bottom: 1px solid #999;}
      
      .all-list{flex-flow:3;font-size:0.36rem;color:#393939;width:100%;height:100%;display:flex;align-items:center;}
      .all-title>i{display:inline-block;height:0.3rem;width:0.3rem;background:url(../../static/images/homepage/pulldown.png);background-size:100%;background-position:center bottom;margin-left:-0.05rem;}
      .all-list>li{height:20%;}
      .noneKtv{margin:0.4rem  auto 0;display:flex;justify-content: center;font-size:0.4rem;}

      .slides{display:flex;justify-content: center;}
      .citySelect{z-index:99;position:relative;background:#ccc;}
      .city-submenu{display:none;}

      .icon-search{background:url(../../static/images/homepage/search.png);width:0.45rem;height:0.4rem;background-size:100%;margin-right:0.3rem;}
